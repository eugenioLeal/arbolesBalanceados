...
public:
    ...
    void insertNonFull(int k); // Utility function
    void splitChild(int i, BTreeNode *y); // Utility function
    void remove(int k);
    void removeFromLeaf(int idx); // Remove the key present in idx-th position in this node which is a leaf
    void removeFromNonLeaf(int idx);// Remove the key present in idx-th position in this node which is a non-leaf node
    int getPred(int idx); // Get the predecessor of the key- where the key is present in the idx-th position in the node
    int getSucc(int idx); // Get the successor of the key- where the key is present in the idx-th position in the node
    void fill(int idx); // Fill up the child node present in the idx-th position in the C[] array if that child has less than t-1 keys
    void borrowFromPrev(int idx); // Borrow a key from the C[idx-1]-th node and place it in C[idx]th node
